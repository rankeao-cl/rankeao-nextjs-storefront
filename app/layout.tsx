...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (slug === "__directory__") {
    return {
      title: "Rankeao | Directorio de Tiendas TCG",
      description: "Directorio central de tiendas de cartas coleccionables.",
    };
  }
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
        ...
        <head>
          <style
            dangerouslySetInnerHTML={{
              __html: `:root, .dark, [data-theme="dark"] { --store-primary: #C52828; --store-secondary: #ffffff; }`,
            }}
          />
        </head>
  ...
  try {
    ...
    const domain = process.env.NEXT_PUBLIC_STOREFRONT_DOMAIN || "rankeao.cl";
    redirect(`${protocol}://${domain}`);
  }
