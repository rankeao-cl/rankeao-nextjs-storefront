    ...
    <footer className="bg-[var(--footer-bg)] mt-auto border-t border-white/5">
      ...
      <div className="store-container pt-14 pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div>
            <h3 className="text-white font-bold text-sm uppercase tracking-wider mb-4">Tienda</h3>
            <ul className="space-y-2.5">
              <li>
                <Link href="/catalogo" className="text-white/50 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[var(--store-primary)]" />
                  Catalogo completo
                </Link>
              </li>
              <li>
                <Link href="/catalogo?sort=recent" className="text-white/50 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[var(--store-primary)]" />
                  Novedades
                </Link>
              </li>
              <li>
                <Link href="/catalogo?on_sale=true" className="text-white/50 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[var(--store-primary)]" />
                  Ofertas
                </Link>
              </li>
              <li>
                <Link href="/catalogo?category=pre-orders" className="text-white/50 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-[var(--store-primary)]" />
                  Preventas
                </Link>
              </li>
            ...
            <ul className="space-y-3">
              {tenant.phone && (
                <li>
                  ...
                  >
                    <svg className="w-4 h-4 text-green-400 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/></svg>
                    {tenant.phone}
                  </a>
                </li>
              )}
              {tenant.email && (
                <li>
                  <a
                    ...
                    className="text-white/50 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2.5"
                  >
                    <svg className="w-4 h-4 text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    {tenant.email}
                  </a>
                </li>
              )}
          ...
          <div>
            <h3 className="text-white font-bold text-sm uppercase tracking-wider mb-4">Ubicacion</h3>
            {tenant.address && (
              <p className="text-white/50 text-sm mb-1 flex items-start gap-2.5">
                <svg className="w-4 h-4 text-[var(--store-primary)] shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                <span>
                  {tenant.address}
        ...
        {tenant.schedules && tenant.schedules.length > 0 && (
          <div className="mt-8 pt-6 border-t border-white/[0.06]">
            <h3 className="text-white font-bold text-sm uppercase tracking-wider mb-3 flex items-center gap-2">
              <svg className="w-4 h-4 text-[var(--store-primary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2">
              {tenant.schedules.map((s, i) => {
                ...
                return (
                  <div key={i} className="bg-white/[0.04] rounded-lg px-3 py-2 text-center">
                    <p className="text-white/70 text-xs font-semibold capitalize mb-0.5">{day.toLowerCase()}</p>
                    <p className={`text-xs font-medium ${s.is_closed ? "text-red-400" : "text-white/50"}`}>
        ...
        <div className="mt-8 pt-6 border-t border-white/[0.06] flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="text-white/30 text-xs font-medium uppercase tracking-wider">Metodos de pago</span>
            <Image
      ...
      <div className="border-t border-white/[0.06] bg-black">
        <div className="store-container py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <p className="text-white/20 text-xs">
            Potenciado por{" "}
            <a href="https://rankeao.cl" target="_blank" rel="noopener noreferrer" className="hover:text-white/40 transition-colors font-medium">
              Rankeao.cl
            </a>
          </p>
        </div>
